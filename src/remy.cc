...
int main( int argc, char *argv[] )
{
  ...
  configuration_range.link_packets_per_ms = make_pair( 1.0, 1.0 ); /* 10 Mbps to 10 Mbps */
  ...
  while ( 1 ) {
	...
	printf( "sender:%s [tp=%f, del=%f]\n", get<0>( x ).c_str(), get<1>( x )/ run.first.link_ppt, get<2>( x ) / run.first.delay );
      }
    }
