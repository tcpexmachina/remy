...
void compile_shader( const GLuint num, const string & source )
{
  ...
  if ( log_length > 1 ) {
    vector<GLchar> buffer( log_length );
    GLsizei written_length;
    glGetShaderInfoLog( num, log_length, &written_length, buffer.data() );

    if ( written_length + 1 != log_length ) {
      throw runtime_error( "GL shader log size mismatch" );
    }

    cerr << "GL shader compilation log: " << string( buffer.data(), log_length ) << endl;
  }
